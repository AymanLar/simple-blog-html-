...
!(function (e) {
    var t = { animation: "fade", from: "50%", to: "110%", content: "Better Add something cool here." };
    e.fn.endpage_box = function (n) {
        var r = e.extend({}, t, n),
            i = e(this),
            s = e(document),
            o = "off",
            ...
            a = "";
        if (i.length < 1) {
            i = e("<div id='endpage-box' class='endpage-box'>" + r.content + "</div>");
            i.hide().appendTo("body");
        } else {
            i.hide();
        }
        i.attr("data-status", "off");
        if (r.from.toString().indexOf("%") >= 0) {
            docHeight = s.height() - e(window).height();
            u = (parseInt(r.from) / 100) * docHeight;
        } else {
            u = r.from;
        }
        if (r.to.toString().indexOf("%") >= 0) {
            docHeight = s.height() - e(window).height();
            a = (parseInt(r.to) / 100) * docHeight;
        } else {
            a = r.to;
        }
        e.fn.animationClassChange = function (t, n, r) {
            var i = e(this);
            if (t == "show") {
                i.addClass("animation-" + n.animation).show();
                i.one("webkitAnimationEnd animationend msAnimationEnd oAnimationEnd animationEnd", function (e) {
                    r = "off";
                    i.data("status", "off");
                    i.removeClass("animation-" + n.animation);
                });
            } else {
                var s = n.animation.replace("In", "Out");
                i.addClass("animation-" + s);
                i.one("webkitAnimationEnd animationend msAnimationEnd oAnimationEnd animationEnd", function (e) {
                    r = "off";
                    i.data("status", "off");
                    i.removeClass("animation-" + s).hide();
                });
            }
        };
        e.fn.animateBox = function (t, n, r) {
            var i = e(this);
            var s = i.position();
            switch (r.animation) {
                case false:
        ...
        s.scroll(function () {
            var e = s.scrollTop();
            if (e >= u && e <= a) {
                if (i.is(":hidden") && i.data("status") == "off") {
                    o = "on";
                    i.data("status", "on");
                    i.animateBox("show", o, r);
                }
            } else {
                if (i.is(":visible") && i.data("status") == "off") {
                    o = "on";
                    i.data("status", "on");
                    i.animateBox("hide", o, r);
                }
            }
        });
    };
